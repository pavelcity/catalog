<h1>Это страница конкретной категории коллекции ))</h1>

<h1>Категория: {{ $catcollect->title }}</h1>
...
@if(true)
<section class="text-gray-600 body-font mb-1 mt-4 sm:mt-6">
  <div class=" px-0 py-2 mx-auto">
    <div class="flex flex-wrap m-0">
     
			<div class="p-2 w-full">
        <div class="block bg-gradient-to-r from-green-400  to-blue-500 px-8 py-10 md:py-20 rounded-lg overflow-hidden text-center relative transition ">
          <h2 class="tracking-widest text-4xl sm:text-8xl title-font font-extrabold text-gray-900 mb-1">{{ $catcollect->title ?? '' }}</h2>
        </div>
      </div>
  ...
  <div class="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
    ...
    <nav class="md:mr-auto md:ml-0 md:py-1 md:pl-4 md:border-l md:border-gray-400	flex flex-wrap items-center text-base justify-center">
      <a href="{{ route('tovary.home') }}" class="mr-5 text-indigo-700 hover:text-indigo-500 text-xl mr-5">Все</a>
			...
			<div class="mr-5 text-gray-900 text-xl mr-5 ">{{ $catcollect->title ?? 'нет категории' }}</div>
...
@empty
	<p class="text-center bg-indigo-400 text-white py-2">ничего нет</p>
@endforelse
@endif
{{-- ----оригинал end NEW--- --}}
