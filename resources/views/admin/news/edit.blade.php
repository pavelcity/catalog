...
@section('title', 'Новости редактировать')
...
@section('content')
	...
	<div class="page-breadcrumb d-none d-md-flex align-items-center mb-3">
		<div class="breadcrumb-title pr-3">Новости</div>
		<div class="pl-3">
			<nav aria-label="breadcrumb">
				<ol class="breadcrumb mb-0 p-0">
					<li class="breadcrumb-item"><a href="{{ route('dashboard.home') }}"><i class='bx bx-home-alt'></i></a>
					</li>
					<li class="breadcrumb-item active" aria-current="page"><a href="{{ route('dashboard.blog.home') }}">Новости</a></li>
	...
	<div class="card radius-5">
		<div class="card-body">
			...
			@if(true)
			<form action="{{ route('dashboard.news.update', $info->id) }}" method="post" autocomplete="off" enctype="multipart/form-data">
				@csrf
		
				<div class="card-title">
					<button type="submit" class="btn btn-primary m-1 px-5 py-2">Сохранить</button>
					<a href="{{ route('dashboard.news.home') }}" class="btn btn-outline-primary m-1 px-5 py-2">Назад</a>
				</div>
				<hr/>
			...
			<div class="tab-content p-3" id="myTabContent">
				...
				<div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
							...
							<br>
								<div class="form-group">
									...
									<input class="form-control form-control-lg" id="title" name="title" type="text" placeholder="Заголовок заметки" value="{{ $info->title }}">
									@error('title')
								...
								<input type="hidden" name="old_pic" value="{{ $info->pic }}">

								<br>
								<label for="short">Главная картинка (превью)</label>
								<div class="mt-0">
									<img src="{{ asset($info->pic ??'uploads/not_image.jpg') }}" width="400" alt="" id="showPic">
								</div>
								...
								<input type="hidden" name="old_smallpic" value="{{ $info->smallpic }}">

								@if(true)
								<label for="short">Картинка (для детальной страницы)</label>
								<div class="mt-0">
									<img src="{{ asset($info->smallpic ?? 'uploads/not_image.jpg') }}" width="400" alt="" id="showPic2">
								</div>
								...
								<div class="form-group">
									...
									<textarea id="short" name="short" class="form-control" rows="10" cols="3">{{ $info->short }}</textarea>
								</div>
								...
								<div class="form-group">
									...
									<textarea id="descr" name="descr" class="form-control" rows="10" cols="3">{{ $info->descr }}</textarea>
								</div>
				...
				<div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab"> 
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="metaTitle" name="metaTitle" type="text" placeholder="meta title" value="{{ $info->metaTitle }}">
						@error('metaTitle')
							<p style="color:red;">{{ $message }}</p>
						@enderror
					</div>
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="description" name="description" type="text" placeholder="meta description" value="{{ $info->description }}">
						@error('description')
							<p style="color:red;">{{ $message }}</p>
						@enderror
					</div>
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="keywords" name="keywords" type="text" placeholder="meta keywords" value="{{ $info->keywords }}">
						@error('keywords')
							<p style="color:red;">{{ $message }}</p>
						@enderror
					</div>
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="author" name="author" type="text" placeholder="meta author" value="{{ $info->author }}">
						@error('author')
							<p style="color:red;">{{ $message }}</p>
						@enderror
					</div>
				...
				<div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
					...
					<input type="hidden" name="old_pic480" value="{{ $info->pic480 }}">
					<br>
					<label for="short">Картинка (для ширины - 480px)</label>
					<div class="mt-0">
						
						<img src="{{ asset($info->pic480 ?? 'uploads/not_image.jpg') }}" width="400" alt="" id="showPic480">
					</div>
					...
					<input type="hidden" name="old_pic414" value="{{ $info->pic414 }}">
					<br>
					<label for="short">Картинка (для ширины - 414px)</label>
					<div class="mt-0">
						
						<img src="{{ asset($info->pic414 ?? 'uploads/not_image.jpg') }}" width="400" alt="" id="showPic414">
					</div>
				...
				<div class="tab-pane fade" id="opengraph" role="tabpanel" aria-labelledby="contact-tab">
					...
					{{-- ogimg 600px --}}
					<br>
					<input type="hidden" name="old_picogimg" value="{{ $info->ogimg }}">

					<label for="short">Картинка ogimg (для соцсетей) - 600x600px</label>
					<div class="mt-0">
						
						<img src="{{ asset($info->ogimg ?? 'uploads/not_image.jpg') }}" width="400" alt="" id="showPicogimg">
					</div>
				...
				<div class="tab-pane fade" id="schemaorg" role="tabpanel" aria-labelledby="contact-tab">
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="sschemaDescription" name="schemaDescription" type="text" placeholder="короткое описание статьи" value="{{ $info->schemaDescription }}">
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="schemaAuthor" name="schemaAuthor" type="text" placeholder="автор статьи" value="{{ $info->schemaAuthor }}">
						@error('schemaAuthor')
							<p style="color:red;">{{ $message }}</p>
						@enderror
					</div>
					...
					<div class="form-group">
						...
						<input class="form-control form-control-lg" id="schemaPublisher" name="schemaPublisher" type="text" placeholder="название компании" value="{{ $info->schemaPublisher }}">
